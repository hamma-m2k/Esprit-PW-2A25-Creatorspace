...
<div class="ig-wrap">
  ...
  <div class="ig-card">
    ...
    <div class="ig-head">
      <form id="form-avatar" method="POST" action="index.php?ctrl=user&action=uploadAvatar" enctype="multipart/form-data" style="display:none;">
        <input type="file" id="input-avatar" name="profile_picture" accept="image/jpeg, image/png, image/webp" onchange="document.getElementById('form-avatar').submit();">
      </form>
      <div class="ig-avatar" onclick="document.getElementById('input-avatar').click();" style="cursor: pointer; position:relative; <?php if($item->getProfilePicture()): ?>background: url('<?= htmlspecialchars($item->getProfilePicture()) ?>') center/cover; color: transparent;<?php endif; ?>" title="Modifier la photo de profil">
        <?php if(!$item->getProfilePicture()): ?><?= $initiales ?><?php endif; ?>
        <div style="position:absolute; bottom:0; right:0; background:#6C3FC5; border-radius:50%; width:30px; height:30px; display:flex; align-items:center; justify-content:center; color:white; font-size:14px; border:3px solid var(--bg); pointer-events:none;">📷</div>
      </div>
