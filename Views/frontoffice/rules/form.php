...
<div style="max-width:680px;margin:0 auto;">
  ...
  <div class="card">
    <div class="card-header">
      ...
      <a href="<?= BASE_URL ?>/rules" class="btn btn-outline btn-sm">← Retour</a>
    </div>

    <form method="POST"
          action="<?= BASE_URL . ($isEdit ? '/rules/update/' . $rule['id'] : '/rules/store') ?>"
          novalidate>
      <?= Csrf::field() ?>
      ...
      document.getElementById('ai_generate_btn').addEventListener('click', async () => {
        ...
        try {
          const res = await fetch('<?= BASE_URL ?>/ai/rule', {
            method: 'POST',
          ...
          document.getElementById('rule_titre').value = data.titre || '';
          document.getElementById('rule_description').value = data.description || '';
          status.textContent = '✓ Champs remplis. Tu peux modifier avant d\'enregistrer.';
        } catch (e) {
      ...
      <div style="display:flex;gap:12px;margin-top:24px;flex-wrap:wrap;">
        ...
        <a href="<?= BASE_URL ?>/rules/add<?= !empty($contrat['id']) ? '?contrat_id='.$contrat['id'] : '' ?>"
           class="btn btn-outline">
          ◆ Page ajout multiple
        </a>
        <a href="<?= BASE_URL ?>/rules" class="btn btn-outline">Annuler</a>
      </div>
